        ...
        while queue and fresh_count > 0:
            for i in range(len(queue)): # This one will run once for all of the rotten fruits that we have accumulated in queue, and for all the newly rotten fruits in the next run, and so on. 
                ...
                for dr, dc in directions:
                    ...
                    if 0 <= nr < ROWS and 0 <= nc < COLS and grid[nr][nc] == 1: # This means the new one is a valid row that is not out of bounds, and is also a fresh fruit, this means that it will become rotten
                        queue.append((nr, nc))
